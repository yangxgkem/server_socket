...
forward_message(int type, bool padding, struct socket_message * result) {
	...
	sm = (struct server_socket_message *)malloc(sz);
	sm->type = type;
	sm->id = result->id;
	sm->ud = result->ud;
	if (padding) {
...
server_socket_send(uint32_t source, int id, void *buffer, int sz) {
	...
	if (wsz < 0) {
		...
		int kb4 = wsz / 1024 / 4;
		if (kb4 % 256 == 0) {
			printf("%d Mb bytes on socket %d need to send out", (int)(wsz / (1024 * 1024)), id);
		}
	}
	return 0;
}
...
void
server_socket_send_lowpriority(uint32_t source, int id, void *buffer, int sz) {
	socket_server_send_lowpriority(SOCKET_SERVER, id, buffer, sz);
}

int 
server_socket_listen(uint32_t source, const char *host, int port, int backlog) {
	return socket_server_listen(SOCKET_SERVER, source, host, port, backlog);
}

int 
server_socket_connect(uint32_t source, const char *host, int port) {
	return socket_server_connect(SOCKET_SERVER, source, host, port);
}

int 
server_socket_bind(uint32_t source, int fd) {
	return socket_server_bind(SOCKET_SERVER, source, fd);
}

void 
server_socket_close(uint32_t source, int id) {
	socket_server_close(SOCKET_SERVER, source, id);
}

void 
server_socket_start(uint32_t source, int id) {
	socket_server_start(SOCKET_SERVER, source, id);
}

void
server_socket_nodelay(uint32_t source, int id) {
